...
19.6 Decisión de migración de infraestructura (2026-07-19)

**Diagnóstico.** El free tier de Render no tiene ninguna región en Sudamérica (sus opciones son Oregon, Ohio, Virginia, Frankfurt y Singapur), mientras que la base de Supabase del proyecto está en `sa-east-1` (São Paulo) y los usuarios reales operan desde Argentina. Cada operación de negocio abre una transacción que hace varias consultas seguidas contra la base (`TenantPrismaService.withTenant`); con Render lejos de Supabase, cada una de esas consultas paga latencia entre continentes, y la suma llegó a superar el timeout por defecto de las transacciones interactivas de Prisma (síntoma observado en producción: `P2028 Transaction not found`, mostrado al usuario como "restricción de datos"). Se aplicaron mitigaciones de código (menos round-trips por transacción, JWT verificado localmente, timeout más generoso — ver commits de la semana del 14/07 al 19/07) que redujeron el impacto, pero la causa de fondo —dónde corre la API respecto a dónde está la base— sigue sin resolverse mientras la API siga en Render.

> **Revisión del 1/08/2026 — esta decisión quedó sin efecto en su parte de proveedor.** El diagnóstico de latencia de esta sección sigue siendo válido; lo que cambió es la respuesta. Se descartó administrar un servidor propio: ver 20.6 punto 4. El texto original se conserva por el razonamiento, que sigue sirviendo el día que haya que elegir dónde correr la API.

**Decisión.** Mover únicamente la API (NestJS) a un host con región en São Paulo, manteniendo Supabase (base + Auth) exactamente como está hoy. Opción elegida: **AWS Lightsail Container Service, con 2 nodos** (redundancia — evita downtime completo ante un reinicio o deploy). Se evaluaron y descartaron para este problema puntual: migrar todo a un único proveedor (Google Cloud o Azure, con Cloud SQL/Postgres Flexible Server + reemplazo completo de Supabase Auth) y mover todo a una región de EE.UU. — ambas alternativas resuelven la multiplicación de round-trips API↔base, pero (a) implican reemplazar Supabase Auth por completo (migración de usuarios + reescritura de la capa `AuthProvider`, ya abstraída para esto pero sin implementar), un proyecto de una a dos semanas en vez de un día, y (b) no mejoran (o directamente no tocan) la latencia usuario→API para los usuarios reales, que están en Argentina y por lo tanto más cerca de São Paulo que de cualquier región de EE.UU.
...
1. **Contratar Supabase Pro — por las copias de seguridad.** Es lo primero de esta lista y lo único que hoy no tiene vuelta atrás: el plan gratuito **no hace backups automáticos**. Si la base se corrompe o alguien borra algo, no hay de dónde volver. Son US$25 al mes y no depende de nada; el resto de la lista, en cambio, depende de esto.
...
2. **`FORCE ROW LEVEL SECURITY` en las 16 tablas, y un rol dedicado para los dos casos legítimos que hoy corren como dueños.**
   ...
   *Orden.* **Va después de tener backups** (punto 0). Es exactamente el tipo de cambio que se hace con red y no sin ella.

3. **Paginación real y KPIs como agregados SQL.** Agendado para el sábado 1 de agosto de 2026, después de contratar Supabase Pro (por los backups) y verificando ambas implementaciones contra datos reales antes de reemplazar la vieja.
...
8. **App nativa**, diferida hasta después de la migración.
