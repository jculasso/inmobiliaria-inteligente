...
export default async function TableroDashboardPage({
  ...
  searchParams: Promise<{ anio?: string; mes?: string; soloMio?: string }>;
}) {
  ...
  const [resumen, resumenAnual] = await Promise.all([
    getKpisResumen(ctx.accessToken, { anio, mes, soloMio }),
    getResumenRango(ctx.accessToken, anio, 1, 12, soloMio),
  ]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <section className="flex flex-col gap-2">
        ...
        <ResumenAcumulado anio={anio} mesSeleccionado={mes} soloMio={soloMio} inicial={resumenAnual} />
      </section>

      <RankingTable anio={anio} mesSeleccionado={mes} soloMio={soloMio} inicial={resumenAnual} />
    </div>
  );
}
